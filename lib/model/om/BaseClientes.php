...
abstract class BaseClientes extends BaseObject  implements Persistent
{
	...
	public function setApellido($v)
	{
		...
		if ($this->apellido !== $v) {
			$this->apellido = $v;
			$this->modifiedColumns[] = ClientesPeer::APELLIDO;
		}
	...
	public function setEdad($v)
	{
		...
		if ($this->edad !== $v) {
			$this->edad = $v;
			$this->modifiedColumns[] = ClientesPeer::EDAD;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->edad = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = ClientesPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ClientesPeer::APELLIDO)) {
			$modifiedColumns[':p' . $index++]  = '`APELLIDO`';
		}
		if ($this->isColumnModified(ClientesPeer::NOMBRE)) {
			$modifiedColumns[':p' . $index++]  = '`NOMBRE`';
		}
		if ($this->isColumnModified(ClientesPeer::EDAD)) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setEdad($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ClientesPeer::APELLIDO)) $criteria->add(ClientesPeer::APELLIDO, $this->apellido);
		if ($this->isColumnModified(ClientesPeer::NOMBRE)) $criteria->add(ClientesPeer::NOMBRE, $this->nombre);
		if ($this->isColumnModified(ClientesPeer::EDAD)) $criteria->add(ClientesPeer::EDAD, $this->edad);
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseClientes:' . $name))
		{
