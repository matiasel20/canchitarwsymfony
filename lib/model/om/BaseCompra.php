...
abstract class BaseCompra extends BaseObject  implements Persistent
{
	...
	public function setClienteid($v)
	{
		...
		if ($this->clienteid !== $v) {
			$this->clienteid = $v;
			$this->modifiedColumns[] = CompraPeer::CLIENTEID;
		}
	...
	public function setProductoid($v)
	{
		...
		if ($this->productoid !== $v) {
			$this->productoid = $v;
			$this->modifiedColumns[] = CompraPeer::PRODUCTOID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->clienteid = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->productoid = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	public function ensureConsistency()
	{
		...
		if ($this->aProducto !== null && $this->productoid !== $this->aProducto->getIdproducto()) {
			$this->aProducto = null;
		}
	} // ensureConsistency
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(CompraPeer::CANTIDAD)) {
			$modifiedColumns[':p' . $index++]  = '`CANTIDAD`';
		}
		if ($this->isColumnModified(CompraPeer::FECHA)) {
			$modifiedColumns[':p' . $index++]  = '`FECHA`';
		}
		if ($this->isColumnModified(CompraPeer::CLIENTEID)) {
			$modifiedColumns[':p' . $index++]  = '`CLIENTEID`';
		}
		if ($this->isColumnModified(CompraPeer::PRODUCTOID)) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CompraPeer::CLIENTEID)) $criteria->add(CompraPeer::CLIENTEID, $this->clienteid);
		if ($this->isColumnModified(CompraPeer::PRODUCTOID)) $criteria->add(CompraPeer::PRODUCTOID, $this->productoid);
	...
	public function setProducto(Producto $v = null)
	{
		if ($v === null) {
			...
			$this->setProductoid($v->getIdproducto());
		}
	...
	{
		if ($this->aProducto === null && ($this->productoid !== null)) {
			$this->aProducto = ProductoQuery::create()->findPk($this->productoid, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseCompra:' . $name))
		{
